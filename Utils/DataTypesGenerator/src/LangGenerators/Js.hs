...
enumDef nameMappers mName' eDef = [untrimming|// enum ${eName'}
  ${eFlds}
  ...
  where
    ...
    humanReadable =
      case () of
        _ | any isJust (snd <$> eValues eDef) -> [untrimming|export function humanReadable${eName'}(__a__) {
         switch(__a__) {
           ${eCases}
         }
         return '';
...
structDef nameMappers mName' sDef = [untrimming|export class ${csn} {
    ...
    toJson() { return JSON.stringify(removeLeadingUnderscore(this)); }
...
fldRepr select nameMappers mName' sn (fn,fld@Field { moduleName, typ, container, dflt }) =
  case select of
    RGetSet -> [untrimming|get ${sfn}() { return this[${jfn'}] }
      set ${sfn}(__a__) {
        ${setter}
      }|]
