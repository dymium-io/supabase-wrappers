...
module LangGenerators.Js where
...
run mDefs installPath@(InstallPath installPath') _rootModule camelizer = do
    logInfo $ "Installing JavaScript modules to " <> displayShow installPath' <> "..."
...
enumDef nameMappers mName' eDef = [untrimming|
  ...
  where
    ...
    f v = [untrimming|export const ${efn} = ${efv}|]
      where
    ...
    humanReadable =
      case () of
        _ | any isJust (snd <$> eValues eDef) -> [untrimming|
...
structDef nameMappers mName' sDef = [untrimming|
  export class ${csn} {
    ...
    toJson() { return JSON.stringify(this).split('"_').join('"') }

    static fromJson(__a__) {
      disableDF()
      let cls = new ${csn}()
...
fldRepr select nameMappers mName' sn (fn,fld@Field { moduleName, typ, container, dflt }) =
  case select of
    RGetSet -> [untrimming|
      get ${sfn}() { return this[${jfn'}] }
      set ${sfn}(__a__) {
        ${setter}
      }
      |]
...
simpleSetter jfn = [untrimming|
  setDirtyFlag()
  this[$jfn] = __a__
  |]

optSetter :: T.Text -> T.Text -> T.Text
optSetter jfn  reader = [untrimming|
  if(__a__ == null) {
...
checkAndSet jfn reader = [untrimming|
...
checkAndSetEnum jfn v d = [untrimming|
