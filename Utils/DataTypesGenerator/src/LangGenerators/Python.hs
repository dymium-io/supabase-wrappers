...
genModule
  ...
  where
    ...
    importEnum = case () of
      _ | null enums' -> ""
        | otherwise   -> [untrimming|from enum import Enum|]

    importDataclass = case () of
      _ | null structs' -> ""
        | otherwise   -> [untrimming|from dataclasses import dataclass|]
...
structDef nameMappers _mName' sDef = [untrimming|
  ...
  where
    ...
    fldDefs = mconcat $ f <$> sFields sDef
      where
        f (fn,Field { moduleName, typ, container }) = [untrimming|${sfn}: ${styp}|]
          where
