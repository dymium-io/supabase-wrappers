...
import (
	...
	"errors"
	"fmt"
	"github.com/apex/log/handlers/es/aliases"
...
// Bulk POST request with the given body.
func (c *Client) BulkWithPipeline(body io.Reader, pipeline string) error {
	inpath := fmt.Sprintf("/_bulk?pipeline=%s", pipeline)
	return c.Request("POST", inpath, body, nil)
}
