...
GOOS=darwin GOARCH=amd64 \
           go build -a -tags netgo -ldflags '-X 'main.MajorVersion=0' -X 'main.MinorVersion=6' -X 'main.ProtocolVersion=6' -w ' 

rm -rf dymium.app
fyne package -os darwin -icon logo.png
cd ../client
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 \
           go build -a -tags netgo -ldflags '-X 'main.MajorVersion=0' -X 'main.MinorVersion=6' -X 'main.ProtocolVersion=6' -w -extldflags "-static"' -o ../GUI/dymiumgui.app/Contents/MacOS/dymium
cd ../GUI
